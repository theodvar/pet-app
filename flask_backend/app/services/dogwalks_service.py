...
def get_all_dogwalks():
    """
    Retrieve all dog walks from the database.
    """
    try:
        connection = get_db_connection()
        cursor = connection.cursor(cursor_factory=RealDictCursor)
        cursor.execute("SELECT id, title, ST_AsGeoJSON(geometry) AS geometry, location, city, comment FROM dogwalks WHERE geometry IS NOT NULL;")
        rows = cursor.fetchall()

        # Convert results to GeoJSON format
        features = []
        for linestring in rows:
            try:
                geometry = json.loads(linestring["geometry"])  # Convert string to JSON object
                features.append({
                        "type": "Feature",
                        "properties": {
                            "id": linestring["id"],
                            "title": linestring["title"],
                            "location": linestring["location"],
                            "city": linestring["city"],
                ...
                print(f"Error parsing geometry for linestring ID {linestring['id']}: {geo_error}")
        return jsonify({
...
def get_dogwalk_by_id(id):
    """
    Retrieve a specific dog walk by ID.
    """
    try:
        connection = get_db_connection()
        cursor = connection.cursor(cursor_factory=RealDictCursor)
        ...
        if row:
            ...
            return jsonify({"error": f"Dog walk with ID {id} not found"}), 404
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
        if connection:
...
def create_dogwalk(data):
    """
    Insert a new dog walk into the database.
    """
    ...
    try:
        ...
        return jsonify({"message": "Dog walk added successfully!"}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
        if connection:
...
def update_dogwalk(id, data):
    """
    Update an existing dog walk record.
    """
    if not data:
    ...
    try:
        ...
        if not updated_id:
            return jsonify({"error": f"No dog walk found with ID {id}"}), 404

        return jsonify({"message": f"Dog walk with ID {id} updated successfully!"}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
        if connection:
...
def delete_dogwalk(id):
    """
    Delete a dog walk record by ID.
    """
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        ...
        if deleted_id:
            return jsonify({"message": f"Dog walk with ID {id} deleted successfully!"}), 200
        else:
            return jsonify({"error": f"No dog walk found with ID {id}"}), 404
    except Exception as e:
        return jsonify({"error": str(e)}), 500
